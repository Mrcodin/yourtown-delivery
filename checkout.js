...
class StripeCheckout {
    ...
    async createOrder(deliveryInfo, cartItems) {
        try {
            ...
            // Calculate taxable items subtotal (non-food items)
            const taxableItemsSubtotal = cartItems.reduce((sum, item) => {
            ...
            if (tip === 0) {
                console.error('⚠️ WARNING: Tip is ZERO! Element might be empty or not found!');
            } else {
            ...
            console.log('📤 SENDING ORDER TO BACKEND WITH TIP:', orderData.tip);
