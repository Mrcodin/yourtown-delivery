...
exports.generatePDFReport = async (req, res) => {
    try {
        ...
        const totalSubtotal = orders.reduce(
            (sum, order) => sum + (order.pricing?.subtotal || 0),
            0
        );
        const totalDeliveryFees = orders.reduce(
            (sum, order) => sum + (order.pricing?.deliveryFee || 0),
            0
        );
        ...
        doc.fontSize(14).fillColor('#7f8c8d').text(period, { align: 'center' });
        ...
        doc.roundedRect(50, summaryY, 495, 140, 5).fillAndStroke('#3498db', '#2980b9');
        ...
        doc.fillColor('#2ecc71').text(`$${totalTax.toFixed(2)}`, 200, summaryY + 105, {
            width: 100,
            align: 'right',
        });
        ...
        doc.fontSize(12).font('Helvetica-Bold');
        doc.text(`$${totalSubtotal.toFixed(2)}`, 440, summaryY + 45, {
            width: 100,
            align: 'right',
        });
        doc.text(`$${totalDeliveryFees.toFixed(2)}`, 440, summaryY + 65, {
            width: 100,
            align: 'right',
        });
        doc.text(`$${totalTips.toFixed(2)}`, 440, summaryY + 85, { width: 100, align: 'right' });
        doc.fillColor('#e74c3c').text(`-$${totalDiscounts.toFixed(2)}`, 440, summaryY + 105, {
            width: 100,
            align: 'right',
        });
        ...
        // PAYMENT METHODS
        doc.fillColor('#2c3e50').fontSize(14).font('Helvetica-Bold').text('Payment Methods', 50);
        ...
        Object.entries(paymentMethods).forEach(([method, count]) => {
            ...
            doc.fillColor('#34495e').text(`${method}: ${count} orders (${percentage}%)`, 60);
            doc.moveDown(0.3);
        });
        ...
        // ORDER STATUS
        doc.fillColor('#2c3e50').fontSize(14).font('Helvetica-Bold').text('Order Status', 50);
        ...
        Object.entries(statusBreakdown).forEach(([status, count]) => {
            ...
            doc.fillColor(color).text(`${status}: ${count} orders (${percentage}%)`, 60);
            doc.moveDown(0.3);
        });
        ...
        // Header background
        doc.rect(50, tableTop - 5, 495, 20).fill('#34495e');
        ...
        sortedOrders.forEach((order, index) => {
            ...
            if (index % 2 === 0) {
                doc.rect(50, currentY - 3, 495, 18).fill('#ecf0f1');
            }
            ...
            doc.fillColor('#27ae60').text(`$${tax.toFixed(2)}`, x, currentY);
            x += colWidths.tax;
        ...
        doc.fontSize(12).fillColor('#7f8c8d').font('Helvetica').text(period, { align: 'center' });

        doc.moveDown(2);

        // Calculate driver earnings
        const driverEarnings = {};
        const DRIVER_PAY_RATE = 4.0; // $4 per delivery
        ...
        orders.forEach(order => {
            if (order.delivery?.driverId) {
                const id =
                    typeof order.delivery.driverId === 'object'
        ...
        if (driversArray.length === 0) {
            ...
            doc.fontSize(10)
                .fillColor('#95a5a6')
                .text('Driver pay stubs will appear here when delivery orders are completed.', {
                    align: 'center',
                });
        } else {
            // Summary statistics
            ...
            doc.roundedRect(50, driverSummaryY, 495, 100, 5).fillAndStroke('#27ae60', '#229954');
            ...
            doc.fontSize(12).font('Helvetica-Bold');
            doc.text(`$${totalDriverPay.toFixed(2)}`, 440, driverSummaryY + 45, {
                width: 100,
                align: 'right',
            });
            doc.text(`$${avgPayPerDriver.toFixed(2)}`, 440, driverSummaryY + 65, {
                width: 100,
                align: 'right',
            });
            ...
            driversArray.forEach((driver, index) => {
                ...
                doc.roundedRect(50, stubY, 495, 85, 3).fillAndStroke('#ecf0f1', '#bdc3c7');
                ...
                doc.fontSize(10)
                    ...
                    .font('Helvetica')
                    .text(
                        `${driver.deliveries} ${driver.deliveries === 1 ? 'delivery' : 'deliveries'} completed`,
                        60,
                        stubY + 32
                    );
                ...
                doc.roundedRect(420, stubY + 10, 115, 65, 3).fillAndStroke('#f8f9fa', '#2c3e50');
                ...
                doc.fontSize(20)
                    .fillColor('#000000')
                    .font('Helvetica-Bold')
                    .text(`$${driver.total.toFixed(2)}`, 425, stubY + 38, {
                        width: 105,
                        align: 'center',
                    });
