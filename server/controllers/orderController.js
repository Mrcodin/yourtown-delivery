...
exports.createOrder = async (req, res) => {
    try {
        ...
        if (promoCodeData && promoCodeData.promoCodeId) {
            ...
            try {
                await PromoCode.findByIdAndUpdate(promoCodeData.promoCodeId, {
                    $inc: { usageCount: 1 },
                    $push: {
                        usedBy: {
                            customerId: customer._id,
...
exports.cancelOrderCustomer = async (req, res) => {
    try {
        ...
        if (order.payment?.stripePaymentIntentId && order.payment.method === 'card') {
            try {
                console.log(
                    `🔄 Processing Stripe refund for payment intent: ${order.payment.stripePaymentIntentId}`
                );
...
exports.cancelOrder = async (req, res) => {
    try {
        ...
        if (order.payment?.stripePaymentIntentId && order.payment.method === 'card') {
            try {
                console.log(
                    `🔄 Admin processing Stripe refund for payment intent: ${order.payment.stripePaymentIntentId}`
                );
        ...
        try {
            ...
            if (order.customerInfo.email) {
                const customerEmailData = orderCancellationEmail(
                    order,
