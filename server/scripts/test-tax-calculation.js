...
async function testTaxCalculation() {
  try {
    ...
    scenario1Items.forEach(item => {
      ...
      subtotal1 += itemTotal;
      console.log(`   ${item.name.padEnd(30)} $${item.price.toFixed(2)} × ${item.qty} = $${itemTotal.toFixed(2)} ${item.taxable ? '(TAXABLE)' : '(TAX-EXEMPT)'}`);
    });
    
    const deliveryFee = 6.99;
    const taxRate = 0.084;
    const taxableItems1 = scenario1Items.filter(i => i.taxable).reduce((sum, i) => sum + (i.price * i.qty), 0);
    ...
    console.log(`   Taxable Amount:     $${taxableAmount1.toFixed(2)} (delivery: $${deliveryFee.toFixed(2)} + taxable items: $${taxableItems1.toFixed(2)})`);
    ...
    const scenario2Items = [
      ...
      { name: bread.name, price: bread.price, qty: 1, taxable: bread.isTaxable },
      { name: toiletPaper.name, price: toiletPaper.price, qty: 1, taxable: toiletPaper.isTaxable },
    ...
    const taxableItems2 = scenario2Items.filter(i => i.taxable).reduce((sum, i) => sum + (i.price * i.qty), 0);
    ...
    console.log(`   Delivery Fee:       $${deliveryFee.toFixed(2)}`);
    console.log(`   Taxable Amount:     $${taxableAmount2.toFixed(2)} (delivery: $${deliveryFee.toFixed(2)} + taxable items: $${taxableItems2.toFixed(2)})`);
    ...
    const scenario3Items = [
      { name: toiletPaper.name, price: toiletPaper.price, qty: 2, taxable: toiletPaper.isTaxable },
    ...
    const taxableItems3 = scenario3Items.filter(i => i.taxable).reduce((sum, i) => sum + (i.price * i.qty), 0);
    ...
    console.log(`   Delivery Fee:       $${deliveryFee.toFixed(2)}`);
    console.log(`   Taxable Amount:     $${taxableAmount3.toFixed(2)} (delivery: $${deliveryFee.toFixed(2)} + taxable items: $${taxableItems3.toFixed(2)})`);
